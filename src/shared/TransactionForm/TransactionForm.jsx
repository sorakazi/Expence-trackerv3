...
const TransactionForm = ({ editingTransaction, close }) => {
  ...
  const createTransaction = async (data) => {
    ...
    try {
      ...
      toast.error(
        "Sorry, registration failed, please added all field to transaction form"
      );
    } finally {
      setIsSubmited(false);
    }
        ...
        <TransactionFormStyle onSubmit={handleSubmit(submit)}>
          ...
          <DateInputWrapper>
            <DateLabel>
              ...
              />
              {errors.date && (
                <ErrorMessage role="alert">Fild required</ErrorMessage>
              )}
              <CalendarIcon>
                <Calendar1 />
              </CalendarIcon>
          ...
          <ParentInputWrapper>
            <OneLabel>
              ...
              <CategoryInput
                ...
                {...register("category", {
                  required: isSubmited ? "Field required" : undefined,
                })}
                placeholder="Different"
                onClick={renderCategoryByType}
              />
              <ErrorMessage>
                {isSubmited && errors.category?.message}
              </ErrorMessage>
            </OneLabel>
          </ParentInputWrapper>
          <ParentInputWrapper>
