...
const DonutChart = () => {
  const generateColorShades = (steps) => {
    ...
    if (steps === 1) {
      colorShades.push(
        `rgba(${colorStartGreen.r}, ${colorStartGreen.g}, ${colorStartGreen.b}, ${colorStartGreen.a})`
      );
      return colorShades;
    }
    ...
    if (steps === 2) {
      ...
      colorShades.push(
        `rgba(${colorMidWhite.r}, ${colorMidWhite.g}, ${colorMidWhite.b}, ${colorMidWhite.a})`
      );
      return colorShades;
    }
    ...
    const greenToWhiteRDiff =
      (colorMidWhite.r - colorStartGreen.r) / (greenToWhiteSteps - 1);
    const greenToWhiteGDiff =
      (colorMidWhite.g - colorStartGreen.g) / (greenToWhiteSteps - 1);
    const greenToWhiteBDiff =
      (colorMidWhite.b - colorStartGreen.b) / (greenToWhiteSteps - 1);

    const whiteToGrayRDiff =
      (colorEndGray.r - colorMidWhite.r) / (whiteToGraySteps - 1);
    const whiteToGrayGDiff =
      (colorEndGray.g - colorMidWhite.g) / (whiteToGraySteps - 1);
    const whiteToGrayBDiff =
      (colorEndGray.b - colorMidWhite.b) / (whiteToGraySteps - 1);
    ...
    for (let i = 0; i < greenToWhiteSteps; i++) {
      ...
      const a =
        colorStartGreen.a +
        (i / (greenToWhiteSteps - 1)) * (colorMidWhite.a - colorStartGreen.a);
    ...
    for (let i = 1; i < whiteToGraySteps; i++) {
      ...
      const a =
        colorMidWhite.a +
        (i / (whiteToGraySteps - 1)) * (colorEndGray.a - colorMidWhite.a);
      colorShades.push(`rgba(${r}, ${g}, ${b}, ${a})`);
    }
  ...
  useEffect(() => {
    const getChartData = () => {
      if (currentTransactions) {
        const currentMonthTransactions = currentTransactions.filter((element) =>
          element.date.startsWith(currentDate)
        );
        let totals = new Map();
        let totalSum = 0;
      ...
      {currentTransactions.length > 0 ? (
        <DonutWrapper>
          <ContentWrapper>
            <Title>
              {" "}
              {transactionsType[0].toUpperCase() +
                transactionsType.substring(1)}{" "}
              categories
            </Title>
            ...
            <DoughnutScrollbar>
              {labelData?.map((label, index) => (
                <ListItem key={index}>
                  ...
                  ></ColoredMarker>
                  <PersengeWrapper>
                    <PersengeSpan> {label}</PersengeSpan>{" "}
                    <span>{percentageData[index]}%</span>
                  </PersengeWrapper>
                </ListItem>
              ))}
            </DoughnutScrollbar>
        ...
        <DonutWrapper>
          <NotTansactionWrapper>
            <NotTansactionTitle>
              Here will be your statistics for the current month
            </NotTansactionTitle>
